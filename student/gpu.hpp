...
class GPU{
  public:
	...
	void	  pullVertex			 (InVertex &i, VertexPullerID activePuller, int offset);
	...
	std::vector<std::unique_ptr<uint8_t[]>> bufferP;
	...
	struct vertexTable{
	 BufferID id;
	 IndexType idxType;
	 std::vector<headSetting> heads;
	};

	std::vector<vertexTable*> vertexSettings;
	...
	struct prg{
		...
		FragmentShader fs;
		Uniforms un;
		std::vector<attribtype> VS2FS;
	};


	std::vector<std::unique_ptr<prg>> program;
	...
	void rasterizace(std::array<OutVertex, 3> primitive);
	void getPrimitive(uint32_t nofVertices, int p, std::vector<std::array<OutVertex, 3>> &primitive);
};
